...
class POIPanel(wx.Panel):
	def __init__(self,parent,ID=-1,label="",pos=wx.DefaultPosition,size=(100,25), panelnum=0):
		...
		self.panelnum = panelnum
		#(1) Create widgets:
		self.matplotlibpanel = MatplotlibPanel(self, size=size, panelnum=panelnum)
		self.canvas          = self.matplotlibpanel.canvas
		self.check           = wx.CheckBox(self, -1, 'Show Labels')
		self.spinx           = wx.SpinCtrl(self, -1, "", min=-1000, max=1000)
		self.spiny           = wx.SpinCtrl(self, -1, "", min=-1000, max=1000)
		### set values:
		self.check.SetValue(True)
		### bind:
		self.check.Bind(wx.EVT_CHECKBOX, self.OnCheck)
		self.spinx.Bind(wx.EVT_SPINCTRL, self.OnSpinX)
		self.spiny.Bind(wx.EVT_SPINCTRL, self.OnSpinY)
		### layout:
		sizerSpin = wx.BoxSizer(wx.HORIZONTAL)
		sizerSpin.Add(self.spinx, 1, flag=wx.CENTER)
		sizerSpin.Add(self.spiny, 1, flag=wx.CENTER)
		sizerV    = wx.BoxSizer(wx.VERTICAL)
		sizerV.Add(self.matplotlibpanel, 5, flag=wx.EXPAND|wx.CENTER)
		sizerV.Add(self.check, 1, flag=wx.CENTER)
		sizerV.Add(sizerSpin, 1, flag=wx.CENTER)
		self.SetSizer(sizerV)
		self.Fit()
		self.Centre()
		
	def cla(self):
		self.matplotlibpanel.cla()
	def plot(self, I, cmax=None):
		self.matplotlibpanel.plot(I, cmax)
	def highlight_poi(self, ind):
		self.matplotlibpanel.highlight_poi(ind)
	def plot_poi(self, xy):
		self.matplotlibpanel.plot_poi(xy)
	def replot_pois(self, pois):
		self.matplotlibpanel.replot_pois(pois)
		
	def set_xy(self, xy):
		if xy==None:
			self.spinx.SetValueString("")
			self.spiny.SetValueString("")
		else:
			self.spinx.SetValue(xy[0])
			self.spiny.SetValue(xy[1])
			
			
	def OnCheck(self, event):
		pyplot.setp(self.matplotlibpanel.ax.texts, visible=event.EventObject.GetValue())
		self.canvas.draw()
	def OnSpinX(self, event):
		x = event.EventObject.GetValue()
		self.parent.poi_move_selected(self.panelnum, x=x)
	def OnSpinY(self, event):
		y = event.EventObject.GetValue()
		self.parent.poi_move_selected(self.panelnum, y=y)
